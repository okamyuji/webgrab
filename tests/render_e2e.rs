...
e2e!(e6_json_continue_command_uses_render, {
    ...
    assert!(
        cc.contains("--render") && !cc.contains("--auto-render"),
        "{cc}"
    );
});
...
e2e!(e7_wait_cap_is_enforced, {
    ...
    assert!(!out.contains(SENTINEL_SLOW), "{out}");
    let ok = (code == 0 && out.contains("[webgrab:short-content"))
        || (code == 6 && err.contains("error=empty"));
    assert!(ok, "code={code} out={out} err={err}");
});

e2e!(e8_auto_render_falls_back_when_chrome_missing, {
    let s = start(all_routes());
    let (code, _out, err) = webgrab_raw(&[
        &s.url("/csr_fast"),
        "--auto-render",
        "--allow-private",
        "--no-robots",
    ...
    assert!(
        err.lines()
            .any(|l| l.starts_with("webgrab: error=empty hint=--raw")),
        "{err}"
    );
    assert!(
        err.lines()
            .any(|l| l.starts_with("webgrab: warn=auto-render-failed reason=render")),
        "{err}"
    );
});
...
e2e!(e10a_no_gain_keeps_static_result, {
    ...
    assert!(
        out.contains("[webgrab:render-status no-gain reason=shorter]"),
        "{out}"
    );
    assert!(
        err.contains("warn=auto-render-no-gain reason=shorter"),
        "{err}"
    );
});
...
e2e!(e10b_no_gain_json_char_counts, {
    ...
    assert!(
        v["rendered_chars"].as_u64().unwrap() < v["static_chars"].as_u64().unwrap(),
        "{out}"
    );
});
